...
\emph{Notações.}
As regiões comuns:
\emph{esfera de Riemann} $\Ch = \C \bigcup \infty$,
\emph{plano superior} $H = \{z\in\C: \Im(z)>0\}$,
\emph{disco} $D_R = \{z\in\C: |z|<R\}$ para $R>0$,
\emph{disco unitário} $D_1$,
\emph{disco perfurado} $D^* = D_1 - \{0\}$,
\emph{anel} $A_R := \{z\in\C: 1/R < |z| < R \}$ para $R\in(1,+\infty)$,
uma região $D\subset \Ch$. Notamos por $\O(D)$ o anel/conjunto das funções holomorfas na $D$.
Usamos $\phi : \Ch\to\Ch$ para \emph{transformações de Möbius} 
...
(a) Achar $g(z) := \frac{1}{2\pi i} \frac{f(w) dw}{w-z}$
para $z\in A - \{|z|=1\}$.
