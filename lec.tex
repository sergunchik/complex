...
Todas as definições e teoremas sobre a convergência,
os limites e as séries reais funcionam em caso complexo
...
\begin{defin}
\begin{enumerate}
\item Digamos que a série $\sum_{n=0}^\infty c_n = c_0 + c_1 + c_2 + \dots$ dos números complexos $c_n\in\C$
\emph{converge} (condicionalmente) a um número complexo $c\in \C$ se a sequência de somas parciais
$s_n := c_0 + c_1 + \dots + c_n$ converge ao $c$.
\item Digamos que a série $c_0 + c_1 + c_2 + \dots$ dos números complexos
\emph{converge ao infinito} $\infty$ se a sequência de somas parciais
converge ao infinito.
\item Digamos que a série $c_0 + c_1 + \dots$ \emph{converge absolutamente} se
a série $|c_0| + |c_1| + \dots$ converge (tem a soma finita).
\item Digamos que a série \emph{diverge absolutamente} se ela não converge absolutamente,
e que a série \emph{diverge}, se ela não converge condicionalmente.
\end{enumerate}
\end{defin}
...
\end{equation}
A série $f(z) = \sum_{n=0}^\infty c_n z^n$
\begin{enumerate}
...
%\footnote{Mas a gente vai provar e usar outros teoremas de Abel sobre estes assuntos.}
...
converge absolutamente\footnote{
Um dos teoremas de Riemann afirma que
se a série $\sum_{n=0}^\infty a_n$ real
converge condicionalmente só (converge, mas não converge absolutamente),
então rearranjando a ordem da somação podemos obter
qualquer número real como a soma. Então para ter algum
sentido de séries $\sum_{n\in\Z} a_n$ que convergem
