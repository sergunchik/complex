...
para qualquer $z\in A$.
\end{teorema}
\begin{proof}
Por qualquer $z$ tal que $r < |z-a| < R$ as séries
$\sum_{n\geq 0} c_n (z-a)^n$
e $\sum_{n<0} c_n (z-a)^n$ convergem absolutamente
pelo critério de Cauchy--Hadamard (cf. \cref{cauchy-hadamard}),
então a série $\sum_{n\in\Z} c_n (z-a)^n$
também converge absolutamente.
O teorema de Abel diz, que as partes principal e regular
convergem no qualquer compacto em $A$.
O \Cref{t:weierstrass} de Weierstrass diz que a soma é holomorfma.
\end{proof}
...
e a série converge absolutamente.
\end{teorema}
\begin{proof}
...
Por qualquer $r<\rho<R$ temos uma cota
...
\end{align*}
Isso significa que a séries
\[ \sum_{n=0}^\infty c_n (z-a)^n  \]
converge absolutamente em disco $\{ z\in\C : |z-a|<R \}$
e a séries
\[ \sum_{n=1}^\infty c_{-n} (z-a)^{-n}  \]
converge absolutamente em ``disco'' \footnote{
Formalmente é um disco perfurado em ponto $\infty$,
...
em disco na esfera de Riemann.}
$\{ z\in\C : |z-a| > r \}$,
e logo a série
\[ \sum_{n\in\Z} c_n (z-a)^n \]
converge absolutamente na interseção deles, i.e. no anel $A = \{ z\in\C : r < |z-a| < R \}$.

Agora vamos ver como a séries $\sum_{n\in\Z} c_n (z-a)^n$ é relacionada com a função original $f(z)$.
...
\[ f(z) = \int_{\gamma_{R'}} \frac{f(w)dw}{w-z} - \int_{\gamma_{r'}} \frac{f(w)dw}{w-z}. \]
...
\[ -\int_{\gamma_{r'}} \frac{f(w)dw}{w-z}
  ...
  = \sum_{n<0} c_n (z-a)^n. \]
Somando, obtemos
\[ f(z) = \int_{\gamma_{R'}} \frac{f(w)dw}{w-z} 
         -\int_{\gamma_{r'}} \frac{f(w)dw}{w-z}
        = \sum_{n\in\Z} c_n (z-a)^n. \]
% finish the proof
...
\[ |c_n| \leq  \frac{\sup_{z\in\gamma_\rho} f(z)}{\rho^n} \]
por todos $\rho > 0$.
\end{proof}
