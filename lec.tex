...
\marginnote{Use a progressão geométrica e teste $M$ de Weierstrass
para provar convergência uniforme.
...
\label{raio-derivada}
Provar que
\[ \limsup |n c_n|^{\frac1{n-1}} = \limsup |c_n|^{\frac{1}n} . \]
\end{problema}
...
\begin{enumerate}
\item \label{abel-1} a série converge absolutamente por qualquer $z$ em disco aberto $\{z\in \C : |z|<R\}$,
e por qualquer $0\leq \rho < R$ a série converge uniforme em disco fechado
$\{z\in\C : |z| \leq \rho \}$;
\item \label{abel-2} para qualquer $|z|>R$ a série diverge desde que os termos dela não são limitados;
\item \label{abel-3} a função $f(z) := \sum_{n=0}^\infty c_n z^n$ é holomorfa em disco aberto $\{z\in \C : |z|<R\}$,
a derivada dela $f'(z)$ é igual a série $\sum_{n=1}^\infty n c_n z^{n-1}$
e tem o mesmo raio de convergência.
\end{enumerate}
\end{teorema}
\begin{proof}
Os itens \eqref{abel-1} e \eqref{abel-2} é \Cref{cauchy-hadamard}.
Agora pelo \Cref{raio-derivada} os raios de convergência
de série e série de derivadas coincidem,
então pelo item \eqref{abel-1} e \Cref{pre-weierstrass} segue o enunciado.
\end{proof}

O círculo $|z|=R$ é chamado o \emph{círculo de convergência}, e o \Cref{t:abel} não especifica
nada sobre convergência de séries ou limite da função $f(z)$ para $z$ neste círculo.\footnote{Mas a gente vai provar
e usar outros teoremas de Abel sobre estes assuntos.}
