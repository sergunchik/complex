...
\begin{problema}[teorema fundamental de álgebra, Argand 1806]
\label{p:tfq-liouville}
Provar que qualquer polinómio $P(z)$ de grau ao menos um tem uma raiz complexa.\footnote{Dica:
suponha que não, prove que $\lim_{z\to\infty} |P(z)|=\infty$ e $\int_{z\in\C} |P(z)| = 1/M >0$,
considere $f(z) = \frac{1}{P(z)}$ e aplique o \cref{t:liouville} de Liouville.}
\end{problema}
...
\end{cor}
\begin{proof}
\eqref{tfq-1}: No \Cref{p:tfa-liouville} derivamos essa parte do \Cref{t:liouville} de Liouville,
...
\eqref{tfq-2} Para a segunda parte\footnote{Na álgebra abstrata essa parte pode ser derivada algebricamente,
...
Pelo \Cref{p:divide-holo} existe uma função i$g(z)$ tal que $f(z) = z g(z)$ e $g$ holomorfa no $B(0,1)$.
